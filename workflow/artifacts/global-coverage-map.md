...
| platform adapters | critical | disposable runtime, rollback | Caddy admitted at Forgejo PR #4: 81.8% statement coverage, 100 race repeats, 20 shuffled race runs, checkpoint injection, and disposable Caddy 2.11.4 install/reopen/rollback proof; PostgreSQL child active | direct OS failures and fixed-struct marshal impossibilities are not line-covered; PostgreSQL evidence is pending; Authentik remains unimplemented; parent workstream and apply remain blocked |
